   ...
   public function index()
	...
	public function ajax_list()
	{
		...
		foreach ($list as $claims) {
			...
			$rct = "<a href='".base_url()."claims.php/facilitator/add_claim_rct/".$claims->rec."' class='btn btn-green btn-icon'><i class='fa fa-cloud-download'></i>".get_phrase('download')."</a>";
			
			if(count($this->db->get_where('apps_files',array('file_group'=>$claims->rec,'upload_type'=>"receipt"))->result_object())=== 0){
				$rct =  "<a href='".base_url()."claims.php/facilitator/add_claim_rct/".$claims->rec."' class='btn btn-orange btn-icon'><i class='fa fa-cloud-upload'></i>".get_phrase('attach_receipt')."</a>";
			}
			
			$row[] = $rct;//17-view
			
			$refNo = "<a href='".base_url()."claims.php/facilitator/add_claim_docs/".$claims->rec."' class='btn btn-green btn-icon'><i class='fa fa-cloud-download'></i>".get_phrase('download')."</a>";
			
			if(count($this->db->get_where('apps_files',array('file_group'=>$claims->rec,'upload_type'=>"approval"))->result_object())=== 0){
				$refNo =  "<a href='".base_url()."claims.php/facilitator/add_claim_docs/".$claims->rec."' class='btn btn-orange btn-icon'><i class='fa fa-cloud-upload'></i>".get_phrase('attach_approval')."</a>";
			}						
			...
			switch($claims->rmks):
				...
				if($claims->reinstatementdate!="0000-00-00" && ($claims->rmks=== 1 || $claims->rmks===3 || $claims->rmks===10)){
					$rmk = get_phrase('reinstated')." ".get_phrase('after')." ".$rmk2."";
				}
			$action_data['rec'] = $claims->rec ;
			$action_data['name'] = $rmk ;
			
			$row[] = $this->load->view("backend/facilitator/medical_action",$action_data,TRUE);//19
	 ...
	 function medical_claims($param1="",$param2="")
    {
		...
		if($param1==="add"){
        	...
        	redirect(base_url().'claims.php/facilitator/add_claim_rct/'.$rec, 'refresh');
		...
		if($param1==='edit'){
			...
			//if($this->db->affected_rows()>0){
				...
				redirect(base_url().'claims.php/facilitator/add_claim_rct/'.$param2, 'refresh');	
			//}
	...
	function decline_claim($rec_id=""){
		...
		if($chk==="0" || ($chk==="2" || $chk==="4") || $reinstatementdate!=="0000-00-00" && ($chk==="1" || $chk==="3")){
