...
$rmks_arr = array('-1'=>"New","0"=>"Submitted","1"=>"Declined By PF","2"=>"Processed By PF","3"=>"Declined By HS","4"=>"Approved By HS","7"=>"Paid","8"=>"Reinstated After PF Decline","9"=>"Reinstated After HS Decline");
		...
		$submitted = $this->db->get_where('claims',array('rmks'=>'0'))->num_rows();				
		
		$reinstated_pf = $this->db->where("rmks =  8")->get('claims')->num_rows();	
		
		$reinstated_hs = $this->db->where("rmks = 9")->get('claims')->num_rows();	
		
		$declined_pf = $this->db->where("rmks = 1")->get('claims')->num_rows();
			
		$declined_hs = $this->db->where("rmks = 3")->get('claims')->num_rows();			
