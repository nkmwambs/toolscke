...
<div class="row">
	<div class="col-sm-12">
		<?php 
			echo form_open(base_url() . 'exams.php/'.$this->session->login_type.'/kcse/', array('id'=>'frm_user_add','class' => 'form-horizontal form-groups-bordered validate',"autocomplete"=>"off",'enctype' => 'multipart/form-data'));
		?>
				...
				<label for="" class="col-xs-4 control-label"><?php echo get_phrase('academic_year');?></label>
					<div class="col-xs-6">
						<select class="form-control" name="acYr" id="acYr">
							...
							<?php
								$yrs = $this->db->select('DISTINCT(acYr)')->get_where('kcse')->result_object();
