...
function cron_insert_parameters_dashboard_table(){
	...
	while ($date1 <= $date2) {
		...
		//$month = date('Y-m-t',strtotime('2018-02-28'));
		...
		// Update run counts if the run exists and has a change
		if($run_obj->num_rows() == 1 && ($month_change->num_rows() > 0 || $fcp_in_dashboard < $count_of_fcps) ){
			$this->db->where(array("month"=>$month));
			$this->db->update('dashboard_run',array('run_count'=>$run_obj->row()->run_count + 1));
		}
		...
		{
			$this->finance_dashboard->build_dashboard_array($month,$count_of_fcps,$fcp_in_dashboard);
		}
