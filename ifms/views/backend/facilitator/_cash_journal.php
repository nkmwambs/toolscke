...
<div class="row">
	<div class="col-sm-12">	
			...
			<?php if($this->finance_model->check_opening_balances($project)===true){ ?>	
					...
					<a href="<?=base_url();?>ifms.php/facilitator/dashboard/<?=$tym;?>" class="btn btn-primary btn-icon pull-left"><i class="fa fa-tachometer"></i><?php echo get_phrase('dashboard');?></a>
				...
				<br/><br/><hr/>
							...
							<body id="">	
								...
								<tr>
									...
									<?php
										...
										$sum_bank_income = $this->finance_model->months_bank_income(date('Y-m-d',$tym),$project);
										...
										$sum_bank_payment = $this->finance_model->months_bank_expense(date('Y-m-d',$tym),$project); 
						...
						<div class="btn btn-info not-selected" id="chkAll"><?=get_phrase('select_vouchers');?></div>
						<hr />
...
function remove_duplicate(project,hID){
            ...
            if(!result) {
					...
					var url = '<?=base_url();?>ifms.php/facilitator/remove_duplicate/'+project+'/'+hID+'/<?=$tym;?>';
