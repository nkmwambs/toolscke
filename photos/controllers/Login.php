...
class Login extends CI_Controller {
	...
	public function validate_login($users_id="",$app_name=""){
        ...
        if ($query->num_rows() > 0) {
					...
					//For ICP Photo tool only 
					if($row->userlevel==='1'){
						$icp_id = $this->db->get_where('users',array('fname'=>$row->fname,'department'=>0))->row()->ID;
						$this->session->set_userdata('project_id', $this->db->get_where('projects',array('partner'=>$icp_id))->row()->num);
					}	
					
					$this->session->set_userdata('app_name', $app_name);	
					
					$this->session->set_userdata('admin_state', $row->admin);
